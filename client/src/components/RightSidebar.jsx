...
const RightSidebar = () => {
  ...
  // Get all the images from the messages and set them in the state
  useEffect(() => {
    setMsgImages(messages.filter(msg => msg.image).map(msg => msg.image))
  },[messages])
 
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full px-2 relative overflow-y-scroll ${selectedUser ? 'max-md:hidden' : ''}`}>
      <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto'>
        ...
        <p className='max-auto'>{selectedUser.bio}</p>
      </div>
